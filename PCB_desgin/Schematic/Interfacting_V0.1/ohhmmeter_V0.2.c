...
void ohm_display(){
	...
	ADC_readd(0,value_from_adc);	/* Read ADC channel 0 */ // this function returns integer of the binary
	value_from_adc_conv = *value_from_adc;
	...
	if(value_from_adc_conv == 0){
		LCD_String("this is zero");
	}else{
		LCD_String("this is non zero");
		LCD_Command(0xC0);
		LCD_PRINT_INT(*value_from_adc , 6);
	}
	
	
	
	if (value_from_adc_conv < 26){ //above 25k ohm
		realvolt = ((value_from_adc_conv * 5.0) / 1023);
		/*debugline*/LCD_print_float(realvolt,1,0);
